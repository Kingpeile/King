...
# ---------------------------------------------------------------------------
# 4) karpathy-perspective — locally-distilled persona Skill (Andrej Karpathy),
#    generated by huashu-nuwa. Source of truth is committed in this repo at
#    .claude/skills/karpathy-perspective; this step mirrors it to the user
#    level so it's globally available across sessions. Marker: the user-level
#    SKILL.md. Re-syncs if the repo copy is newer.
# ---------------------------------------------------------------------------
KP_SRC="${CLAUDE_PROJECT_DIR:-$PWD}/.claude/skills/karpathy-perspective"
KP_DST="$HOME/.claude/skills/karpathy-perspective"
if [ -f "$KP_SRC/SKILL.md" ]; then
  if [ ! -f "$KP_DST/SKILL.md" ] || [ "$KP_SRC/SKILL.md" -nt "$KP_DST/SKILL.md" ]; then
    echo "karpathy-perspective: syncing to user level..."
    mkdir -p "$KP_DST"
    cp -R "$KP_SRC/." "$KP_DST/" 2>/dev/null && echo "karpathy-perspective: ready"
  else
    echo "karpathy-perspective: already installed, skipping"
  fi
fi
