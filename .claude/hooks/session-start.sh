...
# ---------------------------------------------------------------------------
# 3c) ljg-skills (lijigang/ljg-skills, --all) — 继刚的工具箱: ljg-think (追本之箭/
#     深钻), ljg-read, ljg-rank, ljg-roundtable, ljg-card, ljg-paper, ljg-book
#     等 ~22 个。Marker: ljg-think 的 SKILL.md。
# ---------------------------------------------------------------------------
LJG_MARKER="$HOME/.claude/skills/ljg-think/SKILL.md"

if [ -f "$LJG_MARKER" ]; then
  echo "ljg-skills: already installed, skipping"
else
  echo "ljg-skills: installing..."
  npx -y skills@latest add https://github.com/lijigang/ljg-skills -g -a claude-code -s '*' -y >/dev/null 2>&1 || \
    echo "ljg-skills: install failed (continuing)"
  if [ -f "$LJG_MARKER" ]; then
    echo "ljg-skills: ready"
  fi
fi

# ---------------------------------------------------------------------------
# 3d) dbskill (dontbesilent2025/dbskill, --all) — dontbesilent 商业工具箱: dbs
#     (主入口/「帮我看看」), dbs-diagnosis, dbs-decision, dbs-deconstruct,
#     dbs-content 等 ~21 个。Marker: dbs 主入口的 SKILL.md。
# ---------------------------------------------------------------------------
DBS_MARKER="$HOME/.claude/skills/dbs/SKILL.md"

if [ -f "$DBS_MARKER" ]; then
  echo "dbskill: already installed, skipping"
else
  echo "dbskill: installing..."
  npx -y skills@latest add https://github.com/dontbesilent2025/dbskill -g -a claude-code -s '*' -y >/dev/null 2>&1 || \
    echo "dbskill: install failed (continuing)"
  if [ -f "$DBS_MARKER" ]; then
    echo "dbskill: ready"
  fi
fi
