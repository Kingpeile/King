...
After install, available skills include: `diagnose`, `tdd`, `prototype`, `triage`, `to-issues`, `to-prd`, `grill-me`, `grill-with-docs`, `improve-codebase-architecture`, `handoff`, `write-a-skill`, `caveman`, `zoom-out`, `setup-matt-pocock-skills`, `agent-memory`, `huashu-nuwa`, and `karpathy-perspective`. Most are auto-invoked from natural-language intent; `/zoom-out` and `/setup-matt-pocock-skills` must be typed explicitly. `huashu-nuwa` triggers on phrases like 「造skill」「蒸馏XX」「女娲」「XX的思维方式」. `karpathy-perspective` is a persona skill distilled by `huashu-nuwa`; it triggers on 「用 Karpathy 的视角」「Karpathy 会怎么看」「卡帕西」 etc. and role-plays Andrej Karpathy as a thinking advisor for AI/ML/software questions.
